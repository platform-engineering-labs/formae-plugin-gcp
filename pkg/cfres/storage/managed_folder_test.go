...
// TestManagedFolderCreate tests the creation, reading, and deletion of a GCP Storage ManagedFolder
func TestManagedFolderCreate(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	// Create provisioner instance
	managedFolder, err := NewStorageProvisioner(testutil.Config, "GCP::Storage::ManagedFolder")
	require.NoError(t, err, "Failed to create StorageProvisioner")

	// Create a test bucket first (ManagedFolder requires uniform bucket-level access)
	...
	bucketName := fmt.Sprintf("formae-test-mf-%s", uuid.New().String()[:8])
	folderName := fmt.Sprintf("test-folder-%s/", uuid.New().String()[:8])
	t.Logf("Creating test bucket: %s with managed folder: %s", bucketName, folderName)

	ctx := context.Background()

	// Create bucket with uniform bucket-level access (required for ManagedFolder)
	...
	// Test ManagedFolder Create operation
	t.Run("CreateManagedFolder", func(t *testing.T) {
		managedFolderProperties := map[string]interface{}{
			"bucket": bucketName,
			"name":   folderName,
		}

		managedFolderPropsJSON, err := json.Marshal(managedFolderProperties)
		require.NoError(t, err, "Failed to marshal managed folder properties")

		createReq := &resource.CreateRequest{
			ResourceType: "GCP::Storage::ManagedFolder",
			Properties:   managedFolderPropsJSON,
			TargetConfig: testutil.TargetConfig,
		}

		// Create the managed folder
		createResult, err := managedFolder.Create(ctx, createReq)
		...
		t.Logf("ManagedFolder created with native ID: %s", nativeID)
		...
		t.Run("Read", func(t *testing.T) {
			...
			readResult, err := managedFolder.Read(ctx, readReq)
			...
			t.Logf("Read managed folder properties: %+v", readProps)
		})
		...
		t.Run("Delete", func(t *testing.T) {
			...
			deleteResult, err := managedFolder.Delete(ctx, deleteReq)
			...
			t.Logf("ManagedFolder deleted successfully")
		})
		...
		t.Run("VerifyDeleted", func(t *testing.T) {
			...
			t.Logf("Verified managed folder was deleted")
		})
	})
}

func TestManagedFolderNotFound(t *testing.T) {
	managedFolder, err := NewStorageProvisioner(testutil.Config, "GCP::Storage::ManagedFolder")
	require.NoError(t, err)

	// Use proper NativeID format: b/{bucket}/managedFolders/{folderName}
	readReq := &resource.ReadRequest{
		NativeID:     "b/nonexistent-bucket/managedFolders/nonexistent-folder/",
		TargetConfig: testutil.TargetConfig,
		ResourceType: "GCP::Storage::ManagedFolder",
	}

	readResult, err := managedFolder.Read(context.Background(), readReq)
	require.NoError(t, err, "Read should not return error")
	require.NotNil(t, readResult, "Read result should not be nil")
	...
	t.Logf("Verified managed folder not found (error: %s)", readResult.ErrorCode)
}
