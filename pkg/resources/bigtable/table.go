...
// tableResponseTransformer transforms the API response into a normalized format
func tableResponseTransformer(apiResponse map[string]interface{}) map[string]interface{} {
	...
	// Cluster states
	if clusterStates, ok := apiResponse["clusterStates"].(map[string]interface{}); ok {
		props["clusterStates"] = clusterStates
	}

	// Column families
	if columnFamilies, ok := apiResponse["columnFamilies"].(map[string]interface{}); ok {
		families := make(map[string]interface{})
		for familyName, familyData := range columnFamilies {
			if familyMap, ok := familyData.(map[string]interface{}); ok {
				family := make(map[string]interface{})
				...
				// GC rule
				if gcRule, ok := familyMap["gcRule"].(map[string]interface{}); ok {
					family["gcRule"] = gcRule
				}

				families[familyName] = family
			}
		}
		props["columnFamilies"] = families
	}

	// Granularity
	if granularity, ok := apiResponse["granularity"].(string); ok {
		props["granularity"] = granularity
	}

	// Restore info (if table was restored from backup)
	if restoreInfo, ok := apiResponse["restoreInfo"].(map[string]interface{}); ok {
		props["restoreInfo"] = restoreInfo
	}
