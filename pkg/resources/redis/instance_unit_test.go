...
func TestPathBuilder(t *testing.T) {
	...
	if got := redisPathBuilder(ctx); got != "/projects/p/locations/us-central1/instances" {
		t.Errorf("collection path = %q", got)
	}
	ctx.ResourceName = "cache1"
	if got := redisPathBuilder(ctx); got != "/projects/p/locations/us-central1/instances/cache1" {
		t.Errorf("resource path = %q", got)
	}
}

func TestOperationName(t *testing.T) {
	// A create/patch/delete response is an Operation.
...
func TestNativeIDFromMetadataTarget(t *testing.T) {
	...
	got := extractRedisNativeID(map[string]interface{}{
		"name": "projects/p/locations/us-central1/operations/op9",
