...
func TestPathBuilder(t *testing.T) {
	ctx := base.PathContext{Project: "p", Location: "us-central1", ResourceType: "workflows"}
	if got := workflowsPathBuilder(ctx); got != "/projects/p/locations/us-central1/workflows" {
		t.Errorf("collection path = %q", got)
	}
	ctx.ResourceName = "wf1"
	if got := workflowsPathBuilder(ctx); got != "/projects/p/locations/us-central1/workflows/wf1" {
...
func TestOperationName(t *testing.T) {
	...
	if got := extractOperationName(map[string]interface{}{
		"name": "projects/p/locations/us-central1/workflows/wf1",
	}); got != "" {
...
func TestNativeIDFromOperationContext(t *testing.T) {
	...
	ctx := base.PathContext{Project: "p", Location: "us-central1", ResourceType: "workflows", ResourceName: "wf1"}
	got := extractWorkflowsNativeID(
		map[string]interface{}{"name": "projects/p/locations/us-central1/operations/op9", "done": false}, ctx)
	if got != "projects/p/locations/us-central1/workflows/wf1" {
		t.Errorf("native id = %q", got)
	}
}

func TestNativeIDFromMetadataTarget(t *testing.T) {
	// No declared ResourceName: fall back to the operation's metadata.target.
	ctx := base.PathContext{Project: "p", Location: "us-central1", ResourceType: "workflows"}
	got := extractWorkflowsNativeID(map[string]interface{}{
		"name":     "projects/p/locations/us-central1/operations/op9",
		"metadata": map[string]interface{}{"target": "projects/p/locations/us-central1/workflows/wf1"},
	}, ctx)
	if got != "projects/p/locations/us-central1/workflows/wf1" {
...
func TestWorkflowRegistered(t *testing.T) {
	for _, op := range []resource.Operation{
		...
		if !registry.HasProvisioner(WorkflowResourceType, op) {
			t.Errorf("%s not registered for %v", WorkflowResourceType, op)
		}
	}
}
