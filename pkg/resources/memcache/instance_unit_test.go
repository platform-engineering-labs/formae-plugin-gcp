...
func TestPathBuilder(t *testing.T) {
	...
	if got := memcachePathBuilder(ctx); got != "/projects/p/locations/us-central1/instances" {
		t.Errorf("collection path = %q", got)
	}
	ctx.ResourceName = "cache1"
	if got := memcachePathBuilder(ctx); got != "/projects/p/locations/us-central1/instances/cache1" {
...
func TestNativeIDFromOperationContext(t *testing.T) {
	...
	ctx := base.PathContext{Project: "p", Location: "us-central1", ResourceType: "instances", ResourceName: "cache1"}
	got := extractMemcacheNativeID(
		map[string]interface{}{"name": "projects/p/locations/us-central1/operations/op9", "done": false}, ctx)
	if got != "projects/p/locations/us-central1/instances/cache1" {
...
func TestNativeIDFromMetadataTarget(t *testing.T) {
	...
	ctx := base.PathContext{Project: "p", Location: "us-central1", ResourceType: "instances"}
	got := extractMemcacheNativeID(map[string]interface{}{
		"name": "projects/p/locations/us-central1/operations/op9",
		"metadata": map[string]interface{}{
			"target": "projects/p/locations/us-central1/instances/cache1",
		},
	}, ctx)
	if got != "projects/p/locations/us-central1/instances/cache1" {
		t.Errorf("native id from target = %q", got)
	}
}
