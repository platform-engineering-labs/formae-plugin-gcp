...
// VpcAccessAPI - Serverless VPC Access API v1. Resources are location-scoped.
// create/delete are long-running operations (return an Operation to poll);
// get/list return the resource directly.
var VpcAccessAPI = base.APIConfig{
	BaseURL:     "https://vpcaccess.googleapis.com/v1",
	APIVersion:  "v1",
	PathBuilder: vpcAccessPathBuilder,
	Pagination:  &base.PaginationConfig{PageSizeParam: "pageSize"},
}

// VpcAccessOperations - asynchronous (LRO). create/delete return an Operation;
// formae polls Status() until the operation reports done.
var VpcAccessOperations = base.OperationConfig{
	Synchronous:            false,
	OperationIDExtractor:   extractOperationName,
	OperationURLBuilder:    func(_ base.PathContext, opID string) string { return opID },
	NativeIDExtractor:      extractVpcAccessNativeID,
	OperationStatusChecker: checkOperationStatus,
}

// VpcAccessNativeID - full path
// "projects/{project}/locations/{location}/connectors/{name}".
var VpcAccessNativeID = base.NativeIDConfig{
	Format: base.FullPathFormat,
}

// vpcAccessPathBuilder builds
// /projects/{project}/locations/{location}/{resourceType}[/{name}].
func vpcAccessPathBuilder(ctx base.PathContext) string {
...
// extractVpcAccessNativeID builds the resource path. On async create the
...
func extractVpcAccessNativeID(response map[string]interface{}, ctx base.PathContext) string {
	if ctx.ResourceName != "" {
