...
func TestPathBuilder(t *testing.T) {
	ctx := base.PathContext{Project: "p", Location: "us-central1", ResourceType: "triggers"}
	if got := eventarcPathBuilder(ctx); got != "/projects/p/locations/us-central1/triggers" {
		t.Errorf("collection path = %q", got)
	}
	ctx.ResourceName = "trig1"
	if got := eventarcPathBuilder(ctx); got != "/projects/p/locations/us-central1/triggers/trig1" {
...
func TestOperationName(t *testing.T) {
	...
	if got := extractOperationName(map[string]interface{}{
		"name": "projects/p/locations/us-central1/triggers/trig1",
	}); got != "" {
...
func TestNativeIDFromOperationContext(t *testing.T) {
	...
	ctx := base.PathContext{Project: "p", Location: "us-central1", ResourceType: "triggers", ResourceName: "trig1"}
	got := extractEventarcNativeID(
		map[string]interface{}{"name": "projects/p/locations/us-central1/operations/op9", "done": false}, ctx)
	if got != "projects/p/locations/us-central1/triggers/trig1" {
		t.Errorf("native id = %q", got)
	}
}

func TestNativeIDFromMetadataTarget(t *testing.T) {
	// No ResourceName in ctx: fall back to operation metadata.target.
	ctx := base.PathContext{Project: "p", Location: "us-central1", ResourceType: "triggers"}
	got := extractEventarcNativeID(map[string]interface{}{
		"name": "projects/p/locations/us-central1/operations/op9",
		"metadata": map[string]interface{}{
			"target": "projects/p/locations/us-central1/triggers/trig1",
		},
	}, ctx)
	if got != "projects/p/locations/us-central1/triggers/trig1" {
...
func TestTriggerRegistered(t *testing.T) {
	for _, op := range []resource.Operation{
		...
		if !registry.HasProvisioner(TriggerResourceType, op) {
			t.Errorf("%s not registered for %v", TriggerResourceType, op)
		}
	}
}
